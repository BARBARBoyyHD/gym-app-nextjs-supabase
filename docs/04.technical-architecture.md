...
# 🏗️ Technical Architecture — Fitness Membership Web App
...
* **Tables**:
  ...
  * `users` → data member
  * `plans` → harga & durasi membership
  * `payments` → bukti pembayaran
  * `courses` → link embed video + deskripsi materi
...
* **Backend** → Vercel (serverless) atau Railway
...
2. Admin → Form "Add member"
3. Frontend → POST `/users`
4. Backend → insert into `users` table
5. Response success

### 5.2 Add Course Video

1. Admin → Form (title + description + embed URL)
2. Frontend → POST `/courses`
3. Backend → insert into DB
4. Frontend → display iframe

### 5.3 Add Payment

1. Admin → Upload bukti bayar (optional)
2. Frontend → POST `/payments` with file
3. Backend:
...
| API scaling   | Stateless → serverless friendly            |
