...
* `members` 1 — * `memberships` (satu member bisa punya banyak riwayat membership)
...
* `members` 1 — * `payments` (history pembayaran per member)
...
* `CREATE INDEX idx_members_email ON members(email);` -- untuk pencarian cepat
...
* Untuk reporting, tambahkan partial index pada memberships active:
  ...
  ```sql
  CREATE INDEX idx_memberships_active ON memberships(member_id) WHERE status = 'active';
  ```
...
1. **Single active membership per member** (business rule)
...
* **Enable RLS** on `members`, `memberships`, `payments`, `courses`.

### Example policies

* `members` table:

  * Allow read for members with `role = 'admin'` (from JWT)
  * Allow insert/update/delete for admin only

* `memberships`:

  * Admin can full access
  * Optionally allow members to read their own memberships (if members later allowed to login)
...
*Dokumen ini dibuat untuk FitTrack — jika mau ada kolom tambahan (mis. `role` di members, atau `is_active` di courses), bilang saja, saya update.*
