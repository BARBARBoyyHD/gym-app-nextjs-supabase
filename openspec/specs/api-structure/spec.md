...
## Purpose
TBD - created by archiving change standardize-put-delete-id-path. Update Purpose after archive.
## Requirements
...
- **WHEN** a client makes a PUT request to `/api/admin/{name}/put/[id]` with update data
...
- **WHEN** an API route receives a PUT request at `/api/admin/{name}/put/[id]/route.ts`
...
- **WHEN** a client makes a DELETE request to `/api/admin/{name}/delete/[id]`
- **THEN** the system identifies the resource by the ID in the URL path
- **AND** deletes the specified resource

#### Scenario: API processes DELETE request
- **WHEN** an API route receives a DELETE request at `/api/admin/{name}/delete/[id]/route.ts`
- **THEN** the ID parameter is extracted from the URL path
...
- **THEN** the endpoint uses the `/api/admin/{name}/put/[id]/route.ts` structure
...
- **THEN** the endpoint uses the `/api/admin/{name}/delete/[id]/route.ts` structure
- **AND** the resource ID is extracted from the URL path parameter
